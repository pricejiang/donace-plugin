...
| `jobs_completed["plan"] == "ERROR"` | Codex review infra failure (CLI/model version mismatch, plugin missing, parse failure, exception in poller) — review never produced a verdict | **Abort — read `.ai/runs/<id>/plan.json`'s `codex_review.reason` and surface it to the user. They must fix the codex side (e.g. upgrade CLI) and re-run `/donace:plan <run-id>`.** Do NOT dispatch stages — the gate never ran, so the plan is unverified. |
